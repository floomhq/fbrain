...
ICONTEXT_DIR="${ICONTEXT_DIR:-${FBRAIN_DIR:-$HOME/icontext}}"
...
if [ "${#MISSING_DEPS[@]}" -gt 0 ]; then
    ...
    echo ""
    echo "Then re-run: curl -fsSL https://raw.githubusercontent.com/floomhq/icontext/main/get.sh | bash"
    exit 1
fi
# -----------------------------------------------------------------------------
...
if command -v pip3 &>/dev/null && pip3 install -e "$ICONTEXT_DIR" --quiet 2>/dev/null; then
    ...
    ln -sf "$ICONTEXT_DIR/cli.py" "$BIN_DIR/fbrain"
